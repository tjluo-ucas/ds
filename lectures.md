...
数据科学不是模型清单，而是一条带责任的数据生命周期：提出可检验问题 → 理解数据生成与治理 → 建立替代模型 → 评价不确定性与适用边界 → 沟通证据 → 在人工监督下行动。任何主题都必须标明总体、Estimand、数据来源、假设、验证协议和失败条件。
...
可以让 Codex 调查数据和仓库结构、生成候选管线、补充测试、复现实验和审查差异；但 Agent 输出只是一项**等待验证的假设**。数据来源、Split、运行结果、统计主张、因果识别和部署建议必须分别连接到可检查证据，不能用流畅解释替代执行或事实。
